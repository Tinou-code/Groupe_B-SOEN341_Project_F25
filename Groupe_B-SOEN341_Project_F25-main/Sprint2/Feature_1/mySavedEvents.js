...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function renderEvents(eventList) {
    ...
    if (!eventList.length) {
      grid.innerHTML =
        '<div class="no-events">No saved events.</div>';
      return;
    }
    try {
    const resp = await fetch("/saved_events");
    ...
    if (resp.ok && data.ok) {
      ...
      data.events.map(e => {
        if (e.userId === user.id) savedEvents.add(e.eventId);
    })
      console.log("saved events", savedEvents)
      const savedEventsList = eventList.filter(ev => 
        savedEvents.has(ev.id)
      );

      if (!savedEventsList.length) {
        grid.innerHTML = '<div class="no-events">No saved events.</div>';
        return;
      }
  
    grid.innerHTML = savedEventsList
      .map(
        (ev) => `
